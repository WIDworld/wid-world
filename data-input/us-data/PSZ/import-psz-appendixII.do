...
global work_data "C:\Users\Amory\Documents\GitHub\wid-world\work-data"
global us_data "C:\Users\Amory\Documents\GitHub\wid-world\data-input\us-data"
...
* 127 g-percentiles when more than 15 percentile groups available
preserve
	keep if iso=="US"
	gen long p_min = round(1000*real(regexs(1))) if regexm(p, "^p([0-9\.]+)$")
	drop if mi(p_min)
	bys year widcode: egen num=nvals(p_min)
restore
	* -> adiinc992j, p99.992 missing in 1985, etc.

tempfile usineq
save "`usineq'"

// Add US data from WID
use "$work_data/add-us-states-output.dta", clear
gen oldobs=1
keep if iso=="US"
append using "`usineq'"
