...
// Generate average fiscal income
...
// Generate averages for US pre-1962
/*
tempfile data
save `data'
keep if iso=="US" & year<1962
keep if inlist(widcode,"sptinc992j","anninc992i")
gen x=value if widcode=="anninc992i"
bys iso year: egen mean=mean(x)
drop if widcode=="anninc992i"
split p, parse(p)
destring p*, replace
replace value=(value*mean)/((p3-p2)/100)
replace widcode="aptinc992j"
keep iso year p widcode value
tempfile temp
save `temp'
...
drop if substr(widcode,1,6)=="afiinc" & p=="p0p100"
append using `fisc_avg'
