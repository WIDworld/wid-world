...
preserve
	import delimited "$un_data/sna-main/exchange-rate/usd-exchange-rate-$pastyear.csv", clear
	keep if countryarea == "Democratic Republic of the Congo"
	destring year amaexchangerate, replace
	keep year amaexchangerate
	rename amaexchangerate value
	gen iso = "CD"
...
preserve
	import delimited "$un_data/sna-main/exchange-rate/usd-exchange-rate-$pastyear.csv", clear
	keep if countryarea == "South Sudan"
	destring year amaexchangerate, replace force
	keep year amaexchangerate
	rename amaexchangerate value
	gen iso = "SS"
	gen currency = "SSP"
	gen widcode = "xlcusx999i"
	expand 2 if year == 2021, gen(exp)
	replace year = 2022 if exp == 1
	drop exp
	tempfile SSP
	sa `SSP'
restore
...
replace value = 6762275397.72307*54.998*11.83333 if iso == "VE" & year == 2022 // à vérifier
