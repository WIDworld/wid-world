...
gen valuep0p50 = 1-valuep50p100
gen valuep50p90 = 1-valuep90p100 - valuep0p50
drop valuep50p100
...
expand 4 if iso=="FR" & year==1900, gen(dup)
...
drop iso2 id
drop if year<1980 & !(inlist(year, 1820, 1850, 1880, 1900, 1910, 1920) | inlist(year, 1930, 1940, 1950, 1960, 1970)) & missing(valuep0p50) & missing(valuep50p90) & missing(valuep90p100) & missing(valuep99p100) & missing(valuep999p100)


*Calculate growth rates for interpolations within decades
foreach var in valuep90p100 valuep99p100 valuep999p100{
	bys iso (year): gen g_`var' = (`var'-`var'[_n-1])/`var'[_n-1]
}
...
gen top10top1FRSE=(top10top1FR+top10top1SE)/2
gen top10prop=top10top1FR*valuep99p100
...
drop aa
gen top10top01FRSE=(top10top01FR+top10top01SE)/2
...
drop aa
gen top1top01FRSE=(top1top01FR+top1top01SE)/2
***************************************
...
*Top 10% shares
gen aa= valuep90p100/valuep99p100
...
gen aa=valuep99p100/valuep999p100 if iso=="GB" & year==1910
sum aa 
...
gen aa=valuep99p100/valuep999p100
gen bb=1-valuep90p100
...
gen test=valuep0p50+valuep50p90+valuep90p100
...
gen aa=valuep99p100/valuep999p100 if iso=="SE" & year==1900
sum aa
...
gen aa=valuep99p100/valuep90p100 if iso=="SE" & year==1900
sum aa
...
gen bb= valuep99p100/valuep999p100
...
gen missing2020 = 1 if year==2020 & missing(valuep99p100)
encode(iso), gen(id)
xfill missing2020, i(id)
drop id
drop if year==2020 & missing2020==1 
expand 2 if year==2019 & missing2020==1, gen(dup)
...
gen allp=p0p50+p50p90+p90p100
foreach var in p0p50 p50p90	p90p100	p99p100	p999p100{
	replace `var'=`var'/allp if allp!=1 & allp!=.
	}
gen allp2=p0p50+p50p90+p90p100
...
gen sharebot90est=p0p50+p50p90

// bottom 50
gen aa=p0p50/sharebot90est if year==firstp50p90
bys iso: egen withinsb90b50=mean(aa)
drop aa

// middle 40
gen aa=p50p90/sharebot90est if year==firstp50p90
...
gen sharebot99est=1-p99p100

// bottom 50
gen aa=p0p50/sharebot99est if year==firstp99p100
bys iso: egen withinsb99b50=mean(aa)
drop aa

// middle 40
gen aa=p50p90/sharebot99est if year==firstp99p100
bys iso: egen withinsb99m40=mean(aa)
drop aa

// next 9 
gen aa=(p90p100-p99p100)/sharebot99est if year==firstp99p100
...
//gen allp4=p0p50+p50p90+p90p100
//br if allp4!=1
...
// find within bottom 99 share of last full distribution
gen sharebot999est=1-p999p100

// bottom 50
gen aa=p0p50/sharebot999est if year==firstp999p100
bys iso: egen withinsb999b50=mean(aa)
drop aa

// middle 40
gen aa=p50p90/sharebot999est if year==firstp999p100
bys iso: egen withinsb999m40=mean(aa)
drop aa

// next 99
gen aa=(p99p100-p999p100)/sharebot999est if year==firstp999p100
...
// check consistency 
gen allp5=p0p50+p50p90+p90p100
*br if allp5!=1
...
gen p90p100col1=1 if year==1910
...
gen p99p100col1=1 if year==1910
...
gen p999p100col1=1 if year==1910
...
gen p90p100col2=1 if year==1910
...
gen p99p100col2=1 if year==1910
...
gen p999p100col2=1 if year==1910
...
foreach var in p0p50 p50p90 p90p100 p99p100 p999p100{
gen `var'EU=(`var'DE+`var'FR+`var'GB+`var'SE)/4
drop `var'DE `var'GB `var'FR `var'SE
gen `var'EURU=(`var'EU+`var'RU)/2
gen `var'RUIN=(`var'IN+`var'RU)/2
}
...
foreach var in p0p50 p50p90 p90p100 p99p100 p999p100{
gen `var'EGDZTR=(`var'EG+`var'DZ+`var'TR)/3
}
...
foreach var in p0p50 p50p90 p90p100 p99p100 p999p100{
	gen `var'INID=(`var'IN+`var'ID)/2
	drop `var'IN `var'ID
}

gen testINID=`var'p0p50INID+`var'p50p90INID+`var'p90p100INID
...
while `i'==1{
	...
	gen missing = 1 if missing(p90p100)
