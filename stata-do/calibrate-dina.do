...
sort iso year fivelet age pop p
gegen tot = total(n*a/1e5), by(iso year fivelet age pop)
generate s2 = a*n/tot/1e5
...
assert changes > 0 if ///
	...
	!(fivelet == "ptinc" & inlist(iso, "KG", "KZ", "TJ", "TL", "TM", "UZ") & inrange(year, 1980, 1989)) // No overall income available, just shares
/*
